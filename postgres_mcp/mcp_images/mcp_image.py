...
async def process_image_data(
    data: bytes, content_type: str, image_source: str, ctx: Context
) -> Image | None:
    ...
    try:
        ...
        if len(data) <= 1048576:
            try:
                with PILImage.open(BytesIO(data)) as img:
                    ...
                    logger.debug(
                        f"Original image dimensions from {image_source}: {width}x{height}"
                    )
                    logger.debug(
                        f"Image format from PIL: {img.format}, mode: {img.mode}"
                    )
        ...
        temp_path = os.path.join(
            TEMP_DIR, f"temp_image_{hash(image_source)}." + content_type.split("/")[-1]
        )
        ...
        try:
            ...
            with PILImage.open(temp_path) as img:
                ...
                orig_mode = img.mode
                logger.debug(
                    f"Original image dimensions from {image_source}: {orig_width}x{orig_height}"
                )
                logger.debug(
                    f"Large image format from PIL: {orig_format}, mode: {orig_mode}"
                )
            ...
            if max_dimension > 3000:
                ...
                logger.debug(
                    f"Very large image detected ({max_dimension}px), will start with scale factor: {initial_scale}"
                )
                ...
                while True:
                    ...
                    if scale_factor < 1.0:
                        ...
                        current_img = img.resize(
                            (current_width, current_height), PILImage.LANCZOS
                        )
                    else:
                        current_img = img
                    ...
                    current_img.save(
                        img_byte_arr, format="JPEG", quality=quality, optimize=True
                    )
                    ...
                    if len(processed_data) <= 819200:  # 800KB
                        ...
                        logger.debug(
                            f"Returning processed image with format: jpeg, size: {len(processed_data)} bytes"
                        )
                    ...
                    if quality > 20:
                        ...
                        if (
                            ...
                            ctx.error(
                                "Unable to compress image to acceptable size while maintaining quality"
                            )
                            logger.error(
                                f"Failed processing image from {image_source}: dimensions too small"
                            )
                            return None
                        logger.debug(
...
async def process_local_image(file_path: str, ctx: Context) -> Dict[str, Any]:
    ...
    try:
        ...
        logger.debug(
            f"Local image {file_path} has extension '{ext}', mapped to content type '{content_type}'"
        )
        ...
        processed_image = await process_image_data(
            file_data, content_type, file_path, ctx
        )
...
async def process_large_local_image(
    file_path: str, content_type: str, ctx: Context
) -> Dict[str, Any]:
    ...
    try:
        ...
        with PILImage.open(file_path) as img:
            ...
            orig_mode = img.mode
            logger.debug(
                f"Original large local image dimensions from {file_path}: {orig_width}x{orig_height}"
            )
        ...
        if max_dimension > 4000:
            ...
            logger.debug(
                f"Very large image detected, will start with scale factor: {initial_scale}"
            )
...
async def fetch_single_image(
    url: str, client: httpx.AsyncClient, ctx: Context
) -> Dict[str, Any]:
    ...
    try:
        ...
        processed_image = await process_image_data(
            response.content, format_type, url, ctx
        )
...
async def process_images_async(
    image_sources: List[str], ctx: Context
) -> List[Dict[str, Any]]:
...
async def fetch_images(image_sources: List[str], ctx: Context) -> List[Image | None]:
    ...
    try:
        ...
        logger.debug(
            f"Processing {len(image_sources)} image sources: {url_count} URLs and {local_count} local files"
        )
        # Process all images
